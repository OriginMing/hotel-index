<template>
<div>
    <van-swipe :autoplay="-Infinity" class="my-swipe" indicator-color="white">
    ...
    <div class="PRO" v-if="info.length">
       ...
       <van-swipe :autoplay="3000" class="my-swipe pro-banner" indicator-color="black"> <router-link :to="{name:'Details',query:{sid:item.sid}}"  v-for="item of info" :key="item.sid">
            <van-swipe-item style="backgroundColor: transparent;"><img alt="" :src="IMGURL+item.sthumb ">
         </van-swipe-item>
        <div class="pro-banner-name">{{item.sname}}</div>
        <div class="pro-banner-score"><van-rate :value="item.score" color="#ffd21e"
  void-icon="star"
  void-color="#eee" size="0.25rem"/></div>
        <div class="pro-banner-area">{{item.scity}}~{{item.sarea}} <span>RMB</span> <span>{{item.sprice}}</span></div>
       
</router-link>
</van-swipe>
    </div>
   
     <div class="index-content"  v-if="info">
        <div  v-for=" item of info" :key="item.sid" class="content-wrapp"> 
            <div class="content-img" ><img :src="IMGURL+item.sthumb" alt=""></div>
            <div class="content-name" style="margin-top:0.2rem"><span>{{item.sname}}</span></div>
            <div class="content-area" style="font-size:0.12rem;opacity:0.6"><span>{{item.scity}}~{{item.sarea}}</span></div>
             <div class="content-comment"><span><van-rate :value="item.score" color="#ffd21e"
  void-icon="star"
  void-color="#eee" size="0.25rem"/></span></div>
            <div class="content-user">
                <div class="user-head"><span style="color:red">RMB </span><span style="fontSize:0.5rem;">{{item.sprice}} </span><span> 每晚</span></div>
...
import {homestayAll} from "../../http/homestay"
